...
ControladorRegistro.prototype.construirContenido = function()
{
    ...
    {
        for(let campo of this.data.campos)
        {
...
function Ventana(_controlador, _escritorio, _guardada, _estado)
{
    this.titulo = ''
    this.controlador = _controlador
    this.escritorio = _escritorio
    this.guardada = _guardada
    this.redimencionando = false
    this.ajax = new AjaxObjeto()
    this.ajax_inicio = 0
    ...
    if(_estado != undefined && _estado instanceof Object)
    {
        this.estado = _estado.estado
        this.ponerCaja(_estado.caja.x, _estado.caja.y, _estado.caja.w, _estado.caja.h)    
    }
    else
    {
        this.estado = 'normal'
...
Ventana.prototype.ponerCaja = function(_x, _y, _w, _h)
{
    this.caja = {x: _x, y: _y, w: _w, h: _h}
...
{
    this.escritorio.calcularCaja()
    this.calcularCaja()

    if(this.caja.x > this.escritorio.caja.w - 10)this.caja.x = this.escritorio.caja.w - 10
    if(this.caja.y > this.escritorio.caja.h - 50)this.caja.y = this.escritorio.caja.h - 50

    if(this.caja.w > this.escritorio.caja.w - 20)this.caja.w = this.escritorio.caja.w - 20
    if(this.caja.h > this.escritorio.caja.h - 50)this.caja.h = this.escritorio.caja.h - 50

    if(this.caja.x + this.caja.w > this.escritorio.caja.w)this.caja.x = this.escritorio.caja.w - this.caja.w
    if(this.caja.y + this.caja.h > this.escritorio.caja.h)this.caja.y = this.escritorio.caja.h - this.caja.h
...
Ventana.prototype.arrastrar = function(e)
{
    switch(e.type)
    {
        ...
        case 'dragend':
            ...
            x = this.escritorio.dropX
            y = this.escritorio.dropY
...
Ventana.prototype.iter = function()
{
    ...
    this.calcularCaja()
    if(JSON.stringify(this.caja_anterior) != JSON.stringify(this.caja))
    {
        this.redimencionando = true
    }
    else if (this.redimencionando)
    {
        this.redimencionando = false
        this.guardada = false
        this.actualizar()
...
function Escritorio(_id, _agregar)
{
    ...
    this.e.addEventListener("dragover", (this.drop).bind(this))
    this.dropX = 0
    this.dropY = 0
    ...
    */
    ...
    {
        for(let v of ventanas_usuario)
        {
            t = v['clave'].split('#')
            e = JSON.parse(v['estado'])
            if(t.length == 4)
            {
                this.crearVentana(t[1], t[2], t[3], true, e)
            }
        }
    }

}
Escritorio.prototype.calcularCaja = function()
{
    this.caja = {
        x: parseFloat(window.getComputedStyle(this.e).left),
        y: parseFloat(window.getComputedStyle(this.e).top),
        w: parseFloat(window.getComputedStyle(this.e).width),
        h: parseFloat(window.getComputedStyle(this.e).height)
    }
}
Escritorio.prototype.drop = function(e)
{
    if(e.type == 'dragover')
    {
        this.dropX = e.x
...
Escritorio.prototype.crearVentana = function(_tipo, _obj, _param, _guardada = false, _estado)
{
    contr = null
    ...
    nueva = new Ventana(contr, this, _guardada, _estado)
    nueva.cargar()
...
Escritorio.prototype.controlarCreadorVentanas = function()
{
    ...
    else if(this.cargador_datos.estado == 'cargado')
    {
        ...
        if(this.menu_objeto.options.length == 0)
        {
            ...
            {
                opt = document.createElement('option')
                opt.setAttribute('value', objeto.id)
                opt.appendChild(document.createTextNode(objeto.etiqueta))
...
Escritorio.prototype.iter = function()
{
    for(let v of this.ventanas)
    {
        v.iter()
    }
}
...
window.addEventListener('load', function() {
    ...
    setInterval((esc.iter).bind(esc), 200)
